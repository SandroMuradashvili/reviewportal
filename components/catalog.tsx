...
export function ProductCatalog({locale,compact=false}:{locale:Locale;compact?:boolean}){
  ...
  return <div className={`product-grid ${compact?"catalog-compact":""}`}>{items.map(item=>{
    ...
    return <article className={`card product ${kind==="set"?"product-set":""}`} key={"_id" in item?item._id:item.slug}>
      ...
      <div className="product-body">{kind==="individual"?<span className="stock">{stockLabel[locale]} · {stock}</span>:null}<h3>{item.name[locale]}</h3><p>{detail[locale]}</p><div className="catalog-price"><strong>₾{price}</strong>{compare?<del>₾{compare}</del>:null}</div><a className="button" target="_blank" rel="noreferrer" href={whatsapp(`Hello ReviewPortal, I'm interested in ${item.name.en}.`)}>{copy[locale].whatsapp}</a></div>
    </article>})}</div>;
}
...
export function PlanCatalog({locale}:{locale:Locale}){
  ...
  return <div className="plans">{items.map((item,index)=>{const description=item.description?.[locale]??"",features:readonly string[]="featuresLocalized" in item&&item.featuresLocalized?item.featuresLocalized[locale]:Array.isArray(item.features)?item.features:item.features[locale],price=item.priceGel??0,compare=item.compareAtPriceGel,name=item.name[locale];return <article className={`card plan ${index===1?"featured":""}`} key={"_id" in item?item._id:item.slug}><h3>{name}</h3><div className="catalog-price plan-price"><strong>{price>0?`₾${price}`:customLabel[locale]}</strong>{compare?<del>₾{compare}</del>:null}{price>0?<small>{monthLabel[locale]}</small>:null}</div><p>{description}</p><ul className="checks">{features.map(feature=><li key={feature}><Check size={16}/> {feature}</li>)}</ul><a className={`button ${index===1?"light":""}`} href={whatsapp(`Hello ReviewPortal, I'd like to activate the ${item.name.en} package.`)} target="_blank" rel="noreferrer">{activateLabel[locale]}</a></article>})}</div>;
}
