...
export function SiteHeader({ locale }: { locale: Locale }) {
  ...
  return <header className="container nav">
    ...
    <div className="actions"><div className="locale">{locales.map(l=><Link className={l===locale?"current":""} key={l} href={`/${l}`}>{l.toUpperCase()}</Link>)}</div><Link className="button secondary" href="/sign-in">{t.signIn}</Link><Link className="button" href="/sign-in">{t.start}</Link></div>
  </header>;
}
