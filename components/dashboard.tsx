...
export function PublicDashboardPreview({locale}:{locale:Locale}) {
  ...
  return <main className="public-dashboard-preview"><div className="preview-dashboard-head"><div><span className="eyebrow">{tr(locale,"დემო ანალიტიკა","Demo analytics","Демо аналитики")}</span><h1>Gvino &amp; Co.</h1><p>{tr(locale,"30 დღის საჩვენებელი მონაცემები — რეალური ჩანაწერები არ გამოიყენება.","A 30-day example using simulated data—no real records are shown.","Пример за 30 дней на тестовых данных — реальные записи не используются.")}</p></div><span className="preview-badge">{tr(locale,"მხოლოდ სანახავად","Preview only","Только просмотр")}</span></div><div className="metrics dashboard-metrics">{metricRows.map(item=><Metric {...item} key={item.label}/>)}</div><div className="distribution card"><h3>{tr(locale,"შეფასებების განაწილება","Rating distribution","Распределение оценок")}</h3>{ratings.slice().reverse().map((count,index)=>{const rating=5-index;return <div className="bar-row" key={rating}><span>{rating} ★</span><div><i style={{width:`${count}%`}}/></div><strong>{count}</strong></div>})}</div><DailyChart locale={locale} daily={daily}/><PreviewInbox locale={locale} now={now}/></main>;
}

type PreviewRow={id:number;rating:number;comment:string;categories:string[];status:"unread"|"read"|"resolved";submittedAt:number};
const previewComments=[
  [2,"The service was friendly, but our order took much longer than expected.",["Service","Waiting time"]],
  [3,"The wine selection was good. The food temperature could be improved.",["Food quality"]],
  [1,"We waited too long before anyone came to our table.",["Waiting time","Service"]],
  [5,"Excellent atmosphere and a very helpful team.",["Atmosphere"]],
  [4,"Lovely evening. We will visit again.",["Atmosphere","Service"]],
  [2,"The final price was higher than we expected from the menu.",["Price and value"]],
  [3,"Good location, but the order was not completely accurate.",["Order accuracy"]],
  [1,"The table and glasses needed more attention.",["Cleanliness"]],
  [5,"Great food and a smooth experience.",["Food quality","Service"]],
  [2,"The main course arrived after everyone else had finished.",["Waiting time"]],
  [4,"Helpful staff and a comfortable atmosphere.",["Service","Atmosphere"]],
  [3,"It was fine overall, though the portions felt small for the price.",["Price and value"]],
] as const;

function makePreviewRows(now:number):PreviewRow[]{return previewComments.map(([rating,comment,categories],index)=>({id:index+1,rating,comment,categories:[...categories],status:index<3?"unread":index%4===0?"resolved":"read",submittedAt:now-index*7_200_000}))}

function PreviewInbox({locale,now}:{locale:Locale;now:number}){
  const [rows,setRows]=useState(()=>makePreviewRows(now)),[tab,setTab]=useState<"all"|"unread"|"unresolved"|"resolved">("all"),[rating,setRating]=useState(0),[sort,setSort]=useState<"newest"|"oldest"|"rating">("newest"),[opened,setOpened]=useState<PreviewRow|null>(null),[draft,setDraft]=useState("");
  const visible=rows.filter(row=>(tab==="all"||(tab==="unread"?row.status==="unread":tab==="resolved"?row.status==="resolved":row.status!=="resolved"))&&(!rating||row.rating===rating)).sort((a,b)=>sort==="oldest"?a.submittedAt-b.submittedAt:sort==="rating"?b.rating-a.rating:b.submittedAt-a.submittedAt);
  const tabs=["all","unread","unresolved","resolved"] as const;
  const tabText={all:tr(locale,"ყველა","All","Все"),unread:tr(locale,"უნახავი","Unseen","Новые"),unresolved:tr(locale,"მოუგვარებელი","Unresolved","Нерешённые"),resolved:tr(locale,"მოგვარებული","Resolved","Решённые")};
  const statusText={unread:tr(locale,"უნახავი","unseen","новое"),read:tr(locale,"მოუგვარებელი","unresolved","нерешённое"),resolved:tr(locale,"მოგვარებული","resolved","решённое")};
  function open(row:PreviewRow){setRows(current=>current.map(item=>item.id===row.id&&item.status==="unread"?{...item,status:"read"}:item));setOpened({...row,status:row.status==="unread"?"read":row.status});setDraft(row.comment)}
  function update(status:PreviewRow["status"]){if(!opened)return;setRows(current=>current.map(row=>row.id===opened.id?{...row,comment:draft.trim()||row.comment,status}:row));setOpened(null)}
  function exportRows(){const lines=[["Rating","Comment","Categories","Status","Submitted"],...visible.map(row=>[row.rating,row.comment,row.categories.join(" | "),row.status,new Date(row.submittedAt).toISOString()])],csv=lines.map(line=>line.map(value=>`"${String(value).replaceAll('"','""')}"`).join(",")).join("\n"),url=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"})),link=document.createElement("a");link.href=url;link.download="gvino-feedback-demo.csv";link.click();URL.revokeObjectURL(url)}
  return <section className="preview-inbox"><div className="section-head compact"><div><h2 className="dash-section-title">{tr(locale,"უკუკავშირის შემოსულები","Feedback inbox","Входящие отзывы")}</h2><p>{tr(locale,"სცადეთ ფილტრები, რედაქტირება, მოგვარება, წაშლა და ექსპორტი. ცვლილებები გაქრება გვერდის განახლებისას.","Try filtering, editing, resolving, deleting, and exporting. Changes disappear when the page reloads.","Попробуйте фильтры, редактирование, решение, удаление и экспорт. После перезагрузки изменения исчезнут.")}</p></div><button className="button secondary" onClick={()=>setRows(makePreviewRows(now))}><RotateCcw size={16}/>{tr(locale,"აღდგენა","Reset demo","Сбросить")}</button></div><div className="feedback-tabs">{tabs.map(item=><button className={tab===item?"active":""} onClick={()=>setTab(item)} key={item}>{tabText[item]}</button>)}</div><div className="card preview-feedback-toolbar"><div className="preview-rating-filter"><strong>{tr(locale,"შეფასება","Rating","Оценка")}</strong><button className={rating===0?"active":""} onClick={()=>setRating(0)}>{tr(locale,"ყველა","All","Все")}</button>{[1,2,3,4,5].map(item=><button className={rating===item?"active":""} onClick={()=>setRating(item)} key={item}>{item}★</button>)}</div><div className="preview-sort"><strong>{tr(locale,"დალაგება","Sort","Сортировка")}</strong>{(["newest","oldest","rating"] as const).map(item=><button className={sort===item?"active":""} onClick={()=>setSort(item)} key={item}>{item==="newest"?tr(locale,"ახალი","Newest","Новые"):item==="oldest"?tr(locale,"ძველი","Oldest","Старые"):tr(locale,"შეფასება","Rating","Оценка")}</button>)}</div><button className="button secondary" onClick={exportRows}><Download size={16}/>{tr(locale,"ექსპორტი","Export","Экспорт")}</button></div><div className="feedback-cards preview-feedback-list">{visible.map(row=><button className={`card preview-feedback-row ${row.status==="unread"?"is-unread":""}`} onClick={()=>open(row)} key={row.id}><span className="unread-dot"/><strong className="feedback-score">★ {row.rating}</strong><span><b>{row.comment}</b><small>{row.categories.join(" · ")} · {new Intl.DateTimeFormat(locale==="ka"?"ka-GE":locale==="ru"?"ru-RU":"en",{dateStyle:"medium",timeStyle:"short"}).format(row.submittedAt)}</small></span><i className={`pill ${row.status}`}>{statusText[row.status]}</i></button>)}{!visible.length?<div className="card empty-state">{tr(locale,"ამ ფილტრით პასუხები არ არის","No responses match this filter","Нет отзывов по этому фильтру")}</div>:null}</div>{opened?<div className="dashboard-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpened(null)}}><section className="dashboard-modal card preview-feedback-dialog" role="dialog" aria-modal="true"><div className="modal-head"><div><span className="eyebrow">Gvino &amp; Co.</span><h2>★ {opened.rating}</h2></div><button className="icon-button" onClick={()=>setOpened(null)} aria-label="Close"><X size={20}/></button></div><label>{tr(locale,"კომენტარი","Comment","Комментарий")}<textarea value={draft} onChange={event=>setDraft(event.target.value)} maxLength={1000}/></label><div className="preview-dialog-meta">{opened.categories.map(category=><span className="pill" key={category}>{category}</span>)}</div><div className="modal-actions"><button className="button secondary danger-button" onClick={()=>{setRows(current=>current.filter(row=>row.id!==opened.id));setOpened(null)}}><Trash2 size={16}/>{tr(locale,"წაშლა","Delete","Удалить")}</button><button className="button secondary" onClick={()=>update("read")}><Pencil size={16}/>{tr(locale,"ცვლილების შენახვა","Save edit","Сохранить")}</button><button className="button" onClick={()=>update("resolved")}><CheckCircle2 size={16}/>{tr(locale,"მოგვარებულად მონიშვნა","Mark resolved","Решено")}</button></div></section></div>:null}</section>
}
