...
const stateCopy={ka:{loading:"იტვირთება…",missing:"ეს შეფასების ბმული ვერ მოიძებნა.",unavailable:"ეს შეფასების ბმული ამჟამად მიუწვდომელია.",trial:"ამ ბიზნესის საცდელი ლიმიტი ამოიწურა.",google:"დატოვე შეფასება Google-ზე",retry:"გთხოვთ, სცადოთ თავიდან.",privatePrompt:"თუ გსურთ, გაუზიარეთ ბიზნესს პირადი უკუკავშირი. Google-ზე შეფასების დატოვების შესაძლებლობა ყველასთვის ერთნაირად ხელმისაწვდომია.",comment:"რა გსურთ გაუზიაროთ ბიზნესს?"},en:{loading:"Loading…",missing:"This review link could not be found.",unavailable:"This review link is not available right now.",trial:"This business has reached its trial response limit.",google:"Leave a review on Google",retry:"Please try again.",privatePrompt:"If you wish, share private feedback with the business. The option to review on Google is available equally to everyone.",comment:"What would you like the business to know?"},ru:{loading:"Загрузка…",missing:"Ссылка для отзывов не найдена.",unavailable:"Ссылка для отзывов сейчас недоступна.",trial:"Компания достигла лимита пробных ответов.",google:"Оставить отзыв в Google",retry:"Попробуйте ещё раз.",privatePrompt:"При желании отправьте компании личный отзыв. Возможность оставить отзыв в Google одинаково доступна всем.",comment:"Что вы хотели бы сообщить компании?"}};
...
export function FeedbackFlow({locale,business:demoBusiness,slug}:{locale:Locale;business:string;slug:string}){
  ...
  const t=copy[locale],s=stateCopy[locale],[rating,setRating]=useState(0),[comment,setComment]=useState(""),[sent,setSent]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""),[token,setToken]=useState("");
  ...
  async function submit(){if(!rating)return;setBusy(true);setError("");try{if(demo)await new Promise(resolve=>setTimeout(resolve,250));else await submitFeedback({slug,visitTokenHash:token||await getVisitToken(slug),rating,comment:comment||undefined});setSent(true)}catch(reason){setError(reason instanceof Error?reason.message:s.retry)}finally{setBusy(false)}}
  ...
  return <main className={`feedback-page ${demo?"demo-feedback-page":""}`}><section className={`feedback-card ${demo?"demo-feedback-card":""}`} aria-live="polite">{demo?<nav className="demo-locales" aria-label="Language">{(["ka","en","ru"] as const).map(item=><Link key={item} className={item===locale?"active":""} href={`/${item}/r/demo`}>{item.toUpperCase()}</Link>)}</nav>:null}<div className="portal-brand-mark">{demo?<Image src="/brand/secondary.svg" alt="ReviewPortal" width={46} height={46}/>:business.slice(0,1)}</div><h1 className="portal-title">{business}</h1>{sent?<><div className="success-mark">✓</div><h2 className="portal-thanks">{t.thanks}</h2><p className="disclosure">{t.private}</p>{destination?<button className="button google-review" onClick={()=>void goToGoogle()}>{s.google}<ExternalLink size={17}/></button>:null}</>:<><p className="portal-prompt">{prompt}</p><div className="stars" role="radiogroup" aria-label={prompt}>{[1,2,3,4,5].map(n=><button key={n} className={`star ${n<=rating?"active":""}`} onClick={()=>void select(n)} role="radio" aria-checked={rating===n} aria-label={`${n} out of 5`}><Star size={42} fill={n<=rating?"currentColor":"none"}/></button>)}</div>{rating>0&&rating<=3?<><p className="private-followup">{s.privatePrompt}</p><label htmlFor="comment" className="comment-label">{s.comment}</label><textarea id="comment" maxLength={1000} value={comment} onChange={e=>setComment(e.target.value)}/><button className="button full-button" disabled={busy||(!demo&&!token)} onClick={submit}>{busy?"…":t.send}</button></>:null}{error?<p className="auth-error" role="alert">{error}</p>:null}<p className="disclosure">{t.private}<br/>Powered by ReviewPortal · <a href={`/${locale}/privacy`}>Privacy</a></p></>}</section></main>
}
