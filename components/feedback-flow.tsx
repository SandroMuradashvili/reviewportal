...
const stateCopy={ka:{loading:"იტვირთება…",missing:"ეს შეფასების ბმული ვერ მოიძებნა.",unavailable:"ეს შეფასების ბმული ამჟამად მიუწვდომელია.",trial:"ამ ბიზნესის საცდელი ლიმიტი ამოიწურა.",google:"დატოვე შეფასება Google-ზე",retry:"გთხოვთ, სცადოთ თავიდან.",privatePrompt:"ვწუხვართ, რომ გამოცდილება იდეალური არ იყო. მოგვწერეთ რა მოხდა, რათა ბიზნესმა პრობლემის მოგვარება შეძლოს.",comment:"რა შეგვიძლია გავაუმჯობესოთ?",required:"კომენტარი სავალდებულოა",processing:"თქვენს უკუკავშირს ვაგზავნით…",close:"დახურვა",language:"ენა",rating:(n:number)=>`${n} ვარსკვლავი 5-დან`,powered:"შექმნილია ReviewPortal-ით",privacy:"კონფიდენციალურობა"},en:{loading:"Loading…",missing:"This review link could not be found.",unavailable:"This review link is not available right now.",trial:"This business has reached its trial response limit.",google:"Leave a review on Google",retry:"Please try again.",privatePrompt:"We’re sorry your experience wasn’t ideal. Tell us what happened so the business can address it.",comment:"What could we improve?",required:"A comment is required",processing:"Sending your feedback…",close:"Close",language:"Language",rating:(n:number)=>`${n} out of 5`,powered:"Powered by ReviewPortal",privacy:"Privacy"},ru:{loading:"Загрузка…",missing:"Ссылка для отзывов не найдена.",unavailable:"Ссылка для отзывов сейчас недоступна.",trial:"Компания достигла лимита пробных ответов.",google:"Оставить отзыв в Google",retry:"Попробуйте ещё раз.",privatePrompt:"Нам жаль, что ваш опыт оказался неидеальным. Расскажите, что произошло, чтобы компания могла решить проблему.",comment:"Что мы можем улучшить?",required:"Комментарий обязателен",processing:"Отправляем ваш отзыв…",close:"Закрыть",language:"Язык",rating:(n:number)=>`${n} из 5`,powered:"Создано с помощью ReviewPortal",privacy:"Конфиденциальность"}};
...
export function FeedbackFlow({locale,business:demoBusiness,slug}:{locale:Locale;business:string;slug:string}){
  ...
  async function submit(){if(rating<1||rating>3||!comment.trim()){setError(s.required);return}setBusy(true);setError("");try{if(demo)await new Promise(resolve=>setTimeout(resolve,250));else await submitFeedback({slug,visitTokenHash:token||await getVisitToken(slug),rating,comment:comment.trim()});setRevealing(true);await new Promise(resolve=>setTimeout(resolve,2400));setSent(true)}catch{setError(s.retry)}finally{setRevealing(false);setBusy(false)}}
  ...
  return <main className={`feedback-page ${demo?"demo-feedback-page":""}`}><section className={`feedback-card ${demo?"demo-feedback-card":""}`} aria-live="polite" aria-busy={revealing}>{demo?<nav className="demo-locales" aria-label={s.language}>{(["ka","en","ru"] as const).map(item=><Link key={item} className={item===locale?"active":""} href={`/${item}/r/demo`}>{item.toUpperCase()}</Link>)}</nav>:null}<div className="portal-brand-mark">{demo?<Image src="/brand/secondary.svg" alt="ReviewPortal" width={46} height={46}/>:business.slice(0,1)}</div><h1 className="portal-title">{business}</h1>{revealing?<div className="feedback-transition" role="status"><span className="feedback-loader"><i/><i/><i/></span><h2 className="portal-thanks">{s.processing}</h2></div>:sent?<><div className="success-mark">✓</div><h2 className="portal-thanks">{t.thanks}</h2><p className="disclosure">{t.private}</p><button className="button full-button feedback-close" onClick={close}>{s.close}</button>{destination?<button className="google-review-link" onClick={()=>void goToGoogle()}>{s.google}<ExternalLink size={14}/></button>:null}</>:<><p className="portal-prompt">{prompt}</p><div className="stars" role="radiogroup" aria-label={prompt}>{[1,2,3,4,5].map(n=><button key={n} className={`star ${n<=rating?"active":""}`} onClick={()=>void select(n)} role="radio" aria-checked={rating===n} aria-label={s.rating(n)}><Star size={42} fill={n<=rating?"currentColor":"none"}/></button>)}</div>{rating>0&&rating<=3?<><p className="private-followup">{s.privatePrompt}</p><label htmlFor="comment" className="comment-label">{s.comment} <span aria-hidden="true">*</span></label><textarea id="comment" required aria-required="true" maxLength={1000} value={comment} onChange={e=>{setComment(e.target.value);if(e.target.value.trim())setError("")}}/><button className="button full-button" disabled={busy||!comment.trim()||(!demo&&!token)} onClick={submit}>{t.send}</button></>:null}{error?<p className="auth-error" role="alert">{error}</p>:null}<p className="disclosure">{t.private}<br/>{s.powered} · <a href={`/${locale}/privacy`}>{s.privacy}</a></p></>}</section></main>
}
